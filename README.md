...
The first public release focuses on a simple but consequential empirical pattern: a small number of years and country-years carry a disproportionate share of recorded conflict deaths since 1989. The project is designed as a reproducible analytical companion to the published articles, not as a prediction system, causal model, legal classification, or perpetrator-attribution engine.

## Published article

- **Seven years out of thirty-seven. Half the toll.**  
  Article source: [`articles/seven-years-out-of-thirty-seven.md`](articles/seven-years-out-of-thirty-seven.md)  
...
.
├── articles/
│   └── seven-years-out-of-thirty-seven.md
├── data/
│   └── README.md
├── docs/
│   ├── data-sources.md
...
| `notebooks/articles/01_seven_years_out_of_thirty_seven.ipynb` | Article-specific validation notebook for the first published article. It recalculates the headline numbers and exports the figures used in the article. |
...
3. notebooks/core/03_conflictlens_country_year_analysis.ipynb
4. notebooks/articles/01_seven_years_out_of_thirty_seven.ipynb
```

The article-specific notebook expects the analysis-ready panel generated by Notebook 02.
...
The first article's headline figures are computed from UCDP data only. ACLED, V-Dem and WDI are part of the wider ConflictLens country-year panel and are used for complementary context, benchmarking or later analytical axes, not as the primary conflict-death count in the first article.

The period covered in the first article is 1989–2025, which follows the start of UCDP's event-level global coverage. The project does not make claims about earlier mass-fatality conflicts outside this data window.
